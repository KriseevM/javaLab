package lab3.task1.util;

import lab3.task1.logic.AnalogClock;
import lab3.task1.logic.AnalogClockWithSeconds;
import lab3.task1.logic.Clock;
import lab3.task1.logic.IClock;
import org.hibernate.SessionFactory;

import org.hibernate.boot.registry.StandardServiceRegistryBuilder;
import org.hibernate.cfg.Configuration;

public class HibernateSessionFactory {
    private static SessionFactory sessionFactory;
    private HibernateSessionFactory(){

    }
    public static SessionFactory getSessionFactory()
    {
        if(sessionFactory == null)
        {
            try{
                Configuration configuration = new Configuration().configure();
                configuration.addAnnotatedClass(Clock.class);
                configuration.addAnnotatedClass(IClock.class);
                configuration.addAnnotatedClass(AnalogClock.class);
                configuration.addAnnotatedClass(AnalogClockWithSeconds.class);
                StandardServiceRegistryBuilder builder = new StandardServiceRegistryBuilder().
                        applySettings(configuration.getProperties());
                sessionFactory = configuration.buildSessionFactory(builder.build());
            } catch(Exception e)
            {
                throw new RuntimeException(e);
            }
        }
        return sessionFactory;
    }
}
